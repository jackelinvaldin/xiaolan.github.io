    ...
    <div className="relative">
      ...
      >
        {visibleCards.map((item) => (
          <motion.article
            key={item.id}
            className="group overflow-hidden rounded-[28px] border border-white/14 bg-white/[0.09] shadow-[0_16px_50px_rgba(0,0,0,0.24)] [scroll-snap-align:center]"
          ...
          >
            ...
            <div className="p-4">
              ...
              <p className="mt-2 line-clamp-2 text-sm leading-6 text-white/62">{item.description}</p>
            </div>
          </motion.article>
        ))}
      </div>
      ...
      <button
        type="button"
        aria-label="上一张"
        className="absolute left-1 top-1/2 grid size-11 -translate-y-1/2 place-items-center rounded-full border border-white/18 bg-[#0b1020]/72 text-white backdrop-blur-xl transition hover:bg-white hover:text-[#0b1020]"
        onClick={() => move("prev")}
      >
        <CaretLeft size={20} />
      </button>
      <button
        type="button"
        aria-label="下一张"
        className="absolute right-1 top-1/2 grid size-11 -translate-y-1/2 place-items-center rounded-full border border-white/18 bg-[#0b1020]/72 text-white backdrop-blur-xl transition hover:bg-white hover:text-[#0b1020]"
        onClick={() => move("next")}
      >
        <CaretRight size={20} />
      </button>
        ...
        {visibleCards.map((item, index) => (
          <button
            ...
            className={`size-2.5 rounded-full transition ${
              active === index ? "w-8 bg-starlight-pink" : "bg-white/28"
            }`}
            onClick={() => {
              const track = trackRef.current;
              if (!track) return;
